...
<div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm p-4 mb-4">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ url('/admin') }}" target="_blank" class="px-3 py-1.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-md text-xs font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors flex items-center">
                <svg class="w-3.5 h-3.5 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path>
                </svg>
                Admin Panel
...
<div id="confirmation-modal" style="display: none;" class="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center z-[60]">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
        <div class="flex items-center mb-4">
            <div class="flex-shrink-0 w-12 h-12 rounded-full bg-orange-100 dark:bg-orange-900 flex items-center justify-center">
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 flex flex-col" style="max-height: calc(100vh - 300px);">
        ...
        <div class="relative flex-1 min-h-0">
            <div id="command-output" class="bg-gray-50 dark:bg-black text-gray-900 dark:text-green-300 rounded p-3 font-mono text-xs h-full break-words border border-gray-200 dark:border-gray-900" style="overflow-y: scroll; overflow-x: hidden; word-wrap: break-word; white-space: pre-wrap; scrollbar-width: none; -ms-overflow-style: none;">
...
<script>
    ...
    function loadSystemInfo() {
        $.ajax({
            url: "{{ route('management.system-info') }}",
            method: 'GET',
            dataType: 'json',
    ...
    function loadMaintenanceStatus() {
        $.ajax({
            url: "{{ route('management.maintenance.status') }}",
            method: 'GET',
            dataType: 'json',
            success: function(data) {
                if (data.success) {
    ...
    function toggleMaintenanceMode(action, secret) {
        $.ajax({
            url: "{{ route('management.maintenance.toggle') }}",
            method: 'POST',
            headers: {
    ...
    function loadEnvSettings() {
        ...
        $.ajax({
            url: "{{ route('management.env-settings') }}",
            method: 'GET',
            dataType: 'json',
            success: function(data) {
                if (data.success) {
    ...
    function saveEnvSettings() {
        ...
        $.ajax({
            url: "{{ route('management.env-settings.update') }}",
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
    ...
    function executeCommand(command) {
        ...
        const url = new URL("{{ route('management.execute-command') }}");
    ...
    function unlinkStorage() {
        ...
        $.ajax({
            url: "{{ route('management.unlink-storage') }}",
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
